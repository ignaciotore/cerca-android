...
class QuickAccessSettingsActivity:AppCompatActivity(){
    ...
    private fun ui(){
        val scroll=ScrollView(this).apply{setBackgroundColor(Color.parseColor("#F7FAF9"))};val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(20),dp(24),dp(20),dp(36))}
        root.addView(title("Acceso rápido de emergencia",29f));root.addView(body("Configurá formas de activar CERCA sin buscar la app. Siempre tendrás 5 segundos para cancelar antes de enviar la alerta."))
        ...
        val widgetCard=card();widgetCard.addView(title("Botón CERCA en la pantalla principal",21f));widgetCard.addView(body("Agregá un botón grande al inicio del teléfono. Un toque abre la confirmación de 5 segundos y después activa el mismo SOS."));widgetCard.addView(primary("AGREGAR BOTÓN CERCA"){pinWidget()});root.addView(widgetCard,margin())
    ...
    private fun refresh(){val sys=serviceOn();if(!sys)prefs.setEnabled(false);silent=true;sw.isChecked=prefs.enabled()&&sys;silent=false;state.text=if(prefs.enabled()&&sys)"✓ Botón físico activo" else "Requiere habilitar CERCA en Accesibilidad de Android."}
    private fun pinWidget(){if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){val m=getSystemService(AppWidgetManager::class.java);val provider=ComponentName(this,HelpWidgetProvider::class.java);if(m!=null&&m.isRequestPinAppWidgetSupported){m.requestPinAppWidget(provider,null,null);return}};Toast.makeText(this,"Mantené apretada la pantalla principal y elegí Widgets > CERCA.",Toast.LENGTH_LONG).show()}
    ...
    private fun primary(t:String,a:()->Unit)=Button(this).apply{text=t;setTextColor(Color.WHITE);backgroundTintList=ColorStateList.valueOf(Color.parseColor("#0B5960"));setOnClickListener{a()}}
    private fun secondary(t:String,a:()->Unit)=Button(this).apply{text=t;setTextColor(Color.parseColor("#0B5960"));backgroundTintList=ColorStateList.valueOf(Color.parseColor("#E8F2F0"));setOnClickListener{a()}}
